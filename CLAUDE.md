...
- **`cmd/`** — Cobra subcommands (`new`, `open`, `ls`, `write` (aliased `send`), `read` (aliased `logs`), `stop`/`kill`, `rename`, `pin`/`unpin`/`start`/`resume`, `group`, `project`, `config`, `daemon`, `tasks`, `status`, `pr`). Each is mostly argument parsing + picker logic + a call into `internal/`. Most commands accept a session name or fall through to an interactive picker (`internal/picker`); a missing TTY or `CLAUDES_NO_INTERACTIVE=1` skips the picker.
...
- **`internal/schedule`** — Recurring scheduled prompts (what `claudes tasks` now is). A schedule pairs a prompt with a cadence (`interval`/`daily`/`once`); the daemon fires due ones, each spawning an ephemeral `claude -p` session in its own throwaway git worktree (created + torn down via `internal/worktree`), with output teed to a per-run logfile. Durable, mutable state at `~/.cache/claudes/schedules.json` (same flock + atomic-write pattern as `internal/pinned`), plus `~/.cache/claudes/schedules/<id>/<runID>.log`. `internal/schedule/next.go` has the pure `NextFire`/`Due` math; firing/teardown/sweep logic is in `internal/daemon/scheduler.go`. `cmd/tasks.go` is the CLI (`add`/`ls`/`rm`/`enable`/`disable`/`run`/`logs`); the TUI surface is a bottom section in the main `cmd/tui.go` (`cmd/tui_schedules.go`), not a standalone dashboard. interval+daily fire only inside an active window (default 9–18); `once` ignores it and auto-disables. Scheduled-run sessions carry `CLAUDES_SCHEDULED=1` and are filtered out of `session.List` so they don't show as agents. Spawned with `claude -p --permission-mode auto` (overridable per-schedule) so headless runs don't hang on permission prompts.

- **`internal/worktree`** — Thin git-worktree plumbing for scheduled runs: `RepoRoot`/`SiblingPath`/`Create`/`Teardown`. Each run gets `<repo>/../<repo>-worktrees/<name>-<ts>` on branch `claudes-sched-<id>-<ts>`; teardown is idempotent (force-remove + prune + `branch -D`) so the daemon's restart sweep can retry.
