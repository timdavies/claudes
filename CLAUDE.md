...
- **`cmd/`** — Cobra subcommands (`new`, `open`, `ls`, `send`, `logs`, `stop`/`kill`, `rename`). Each is mostly argument parsing + picker logic + a call into `internal/`. Most commands accept a session name or fall through to an interactive picker (`internal/picker`); a missing TTY or `CLAUDES_NO_INTERACTIVE=1` skips the picker.
